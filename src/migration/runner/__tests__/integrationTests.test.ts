...
const validateDatabaseObjects = async (): Promise<boolean> => {
  const pool = new Pool({ connectionString: DB_CONNECTION_STRING });
  try {
    log('Validating database objects...');
...
const listTablesInSchema = async (): Promise<string[]> => {
  const pool = new Pool({ connectionString: DB_CONNECTION_STRING });
  try {
    log(`Listing tables in schema ${TEST_SCHEMA}...`);
...
describe('Migration Runner Integration Tests', () => {
  ...
  test('should generate and apply migrations from schema file', async () => {
    ...
    log(`Migration generated with ${migration.steps.length} steps.`);
    ...
    log('Initializing migration runner...');
    ...
    log(`Migration successful! Applied versions: ${result.appliedMigrations.join(', ')}`);
    ...
    log(`Rollback successful! Rolled back versions: ${rollbackResult.rolledBackMigrations.join(', ')}`);
    
    // Verify tables are gone after rollback
    const tablesAfterRollback = await listTablesInSchema();
    
    // After rollback, only the migrations table should remain
    expect(tablesAfterRollback.length).toBe(1);
    expect(tablesAfterRollback[0].toLowerCase()).toBe('schema_migrations');
